...
VertexOutput NormalSpecularReflectVS( VertexInput Input ) 
{
	...
	float3 WorldViewNormal = normalize( Output.WorldViewNormal );
	Output.Reflect = normalize( reflect( Output.WorldViewEyeVec, WorldViewNormal ) );
...
float4 CalcNormalSpecularReflectColor( VertexOutput Input )
{
	...
	float4 ReflectTex = texCUBE( g_ReflectSampler, Input.Reflect );
	
	SpecularTex.xyz *= ( g_ReflectPower * ReflectTex.xyz );
