...
float4 CalcDiffuseColor( VertexOutput Input )
{
	...
	float2 AddCoord = float2( frac( g_ScrollSpeedU * g_fElapsedTime ), frac( g_ScrollSpeedV * g_fElapsedTime ) ) * MaskTex.r;
	float4 DiffuseTex = tex2D( g_DiffuseSampler, Input.TexCoord0 + AddCoord );
