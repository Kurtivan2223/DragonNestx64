...
float4 CalcDiffuseColor( VertexOutput Input )
{
	...
	float2 TexCoord = Input.TexCoord0 + float2( frac( g_ScrollSpeedU * g_fElapsedTime ), frac( g_ScrollSpeedV * g_fElapsedTime ) ) * MaskTex.r;
	...
	float4 EmissiveTex = tex2D( g_EmissiveSampler, TexCoord ) * fEmissive;
