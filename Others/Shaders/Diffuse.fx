...
VertexOutputShadow DiffuseShadowVS( VertexInput Input ) 
{
	...
	const float fAvoidBackProjectionValue = 0.5f;
	if ( Input.Normal.y  < fAvoidBackProjectionValue )  {
		Output.LightSpacePos = float4(-10,-10,-10000,1);
	}
...
VertexOutput DiffuseVS( VertexInput Input ) 
{
	...
	float3 WorldViewPos = mul( float4( Input.Position.xyz , 1.0 ) , g_WorldViewMat );
	Output.Position = mul( float4( WorldViewPos, 1.0 ) , g_ProjMat );
...
PixelOutput DiffuseShadowPS( VertexOutputShadow Input )
{
	PixelOutput Output;
