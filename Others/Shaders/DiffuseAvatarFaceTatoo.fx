...
texture2D g_TatooTex : TATOOTEXTURE
< 
	string UIName = "Tatoo Texture";
>;
sampler2D g_TatooSampler = sampler_state
{
	texture = < g_TatooTex >;
	MinFilter = Linear;
	MagFilter = Linear;
...
float4 CalcDiffuseColor( VertexOutput Input )
{
	...
	float4 TatooTex = tex2D( g_TatooSampler, Input.TexCoord0.xy );
	...
	DiffuseTex.xyz = lerp( DiffuseTex.xyz, TatooTex.xyz, TatooTex.a );

	return Input.Color * DiffuseTex;
}
#endif
