...
texture2D g_BlendTex : BLENDTEXTURE
< 
...
float g_BlendSpeed : BLENDSPEED
<
    string UIName = "Blend Speed";
> = { 0.25f };
...
PixelOutput SkyBoxPS( VertexOutput Input ) : COLOR
{
	...
	float4 layerTexture = tex2D( g_BlendSampler, Input.TexCoord0 );
	
	float fBlendRatio = sin(g_fElapsedTime * g_BlendSpeed)*0.5+0.5;
		
	Output.Color.rgb = lerp( Output.Color.rgb, layerTexture.rgb, fBlendRatio);
